...
public:
  ...
  void set_max_framerate(float max_framerate) { this->max_framerate_ = max_framerate; }
  ...
  void set_dynamic_resolution(framesize_t framesize);
  framesize_t get_resolution() const { return this->framesize_; }
  static const char *get_resolution_name(framesize_t fs);

  void cold_restart(uint32_t power_down_ms = 2000);
  void reset_sensor();
  void restore_factory_defaults();
  void apply_sensor_settings();
  ...
  struct CameraSettings {
    ...
    int8_t contrast{0};
    int8_t saturation{0};
    int8_t special_effect{0};
    int8_t whitebal{1};
    int8_t awb_gain{1};
    int8_t wb_mode{0};
    int8_t exposure_ctrl{1};
    int8_t aec2{1};
    int8_t ae_level{0};
    uint16_t aec_value{300};
    int8_t gain_ctrl{0};
    int8_t agc_gain{6};
    int8_t gainceiling{0};
    int8_t bpc{0};
    int8_t wpc{1};
    int8_t raw_gma{1};
    int8_t hmirror{0};
  ...
  const CameraSettings &get_factory_defaults() const { return this->factory_defaults_; }

protected:
  CameraSettings factory_defaults_{};
  uint32_t xclk_freq_hz_{16000000};
  float  max_framerate_{5.0};
