...
void CRtspSession::Handle_RtspDESCRIBE()
{
    ...
    if (m_URLPreSuffix[0] == '\0' && m_URLSuffix[0] == '\0') m_StreamID = 0;
    ...
    u_short w = m_Streamer ? m_Streamer->GetWidth() : 640;
    u_short h = m_Streamer ? m_Streamer->GetHeight() : 480;
    if (w == 0) w = 640;
    if (h == 0) h = 480;
...
void CRtspSession::Handle_RtspSETUP()
{
    ...
    IPADDRESS client_ip;
    IPPORT client_port;
    socketpeeraddr(m_RtspClient, &client_ip, &client_port);

    if (m_TcpTransport)
        snprintf(Transport,sizeof(Transport),"RTP/AVP/TCP;unicast;interleaved=0-1");
    else
        snprintf(Transport,sizeof(Transport),
                 "RTP/AVP;unicast;destination=%s;source=0.0.0.0;client_port=%i-%i;server_port=%i-%i",
                 inet_ntoa(client_ip),
                 m_ClientRTPPort,
                 m_ClientRTCPPort,
