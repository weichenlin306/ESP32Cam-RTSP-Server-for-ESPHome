...
#define MAX_FRAGMENT_SIZE 1100 // FIXME, pick more carefully
    ...
    RtpBuf[20] = 0x00;                               // type 0 for VLC / live555 compatibility
    ...
    while(bytes - *start < *len) {
        ...
        if(typecode == marker) {
            ...
            *len -= skipped;
            ...
            {
                break;   // no data to skip for standalone markers
            }
            default:
            {
...
void skipScanBytes(BufPtr *start) {
    ...
    while(true) { // FIXME, check against length
        while(*bytes++ != 0xff);
        uint8_t marker = *bytes++;
        // Ignore 0x00 (escaped FF) and Restart Markers (0xD0 - 0xD7)
