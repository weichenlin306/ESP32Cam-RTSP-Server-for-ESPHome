...
void OV2640::capture_task_func(void *param)
{
    OV2640 *self = static_cast<OV2640 *>(param);
    while (self->capture_running) {
        camera_fb_t *new_fb = esp_camera_fb_get();
        if (new_fb != NULL) {
            if (self->fb_mutex && xSemaphoreTake(self->fb_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
                if (self->fb != NULL) {
                    esp_camera_fb_return(self->fb);
                }
                self->fb = new_fb;
                xSemaphoreGive(self->fb_mutex);
            } else {
...
bool OV2640::lockFrame(void)
{
    if (fb_mutex && xSemaphoreTake(fb_mutex, pdMS_TO_TICKS(50)) == pdTRUE) {
...
{
    // Non-blocking: background capture_task_func automatically populates fb
    if (fb == NULL) {
        runIfNeeded();
    }
}
...
esp_err_t OV2640::init(camera_config_t config)
{
    ...
    if (psram_size > 0) {
        ...
        _cam_config.grab_mode = CAMERA_GRAB_WHEN_EMPTY;
    ...
    xTaskCreatePinnedToCore(OV2640::capture_task_func, "cam_capture_task", 4096, this, 5, &capture_task_handle, 0);
    ESP_LOGI(TAG, "Background camera capture task created on Core 0.");
