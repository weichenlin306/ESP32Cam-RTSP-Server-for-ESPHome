...
void Esp32camRtsp::setup() {
  ...
  s->set_special_effect(
      s,
      special_effect_); // 0 to 6 (0 - No Effect, 1 - Negative, 2 - Grayscale, 3
                        // - Red Tint, 4 - Green Tint, 5 - Blue Tint, 6 - Sepia)
  s->set_whitebal(s, whitebal_); // 0 = disable , 1 = enable
  s->set_awb_gain(s, awb_gain_); // 0 = disable , 1 = enable
  s->set_wb_mode(s, wb_mode_);   // 0 to 4 - if awb_gain enabled (0 - Auto, 1 -
  ...
  if (server_socket_ >= 0) {
    int enable = 1;
    setsockopt(server_socket_, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int));
    ...
    if (bind(server_socket_, (struct sockaddr *)&server_addr,
      ...
      ESP_LOGE(TAG, "Failed to bind RTSP port");
    } else {
  ...
  xTaskCreatePinnedToCore(rtsp_task, "rtsp_task",
                          4096, // Stack size
                          this, // Passing instance pointer
                          5,    // Priority
                          NULL,
                          1 // Core 1
  );
...
void Esp32camRtsp::rtsp_task(void *pvParameters) {
  Esp32camRtsp *server = (Esp32camRtsp *)pvParameters;
  while (true) {
    if (session != nullptr) {
      // If we have an active client connection, just service that until gone
      session->handleRequests(
          0); // We don't use a timeout here,
              // instead we send only if we have new enough frames
      now = esphome::millis();
      if (now >= lastFrameTime + msec_per_frame ||
          /*handle clock rollover*/ now < lastFrameTime) {
        
        // Compute real processing start time context if needed
        uint32_t frame_start = lastFrameTime;
        
        session->broadcastCurrentFrame(now);
        lastFrameTime += msec_per_frame;

        // Check if we are overrunning our max frame rate
        now = esphome::millis();
        if (now > lastFrameTime + msec_per_frame) {
          ESP_LOGCONFIG(TAG,
                        "WARNING: The real time-per-frame, %d ms, exceeds that "
                        "of the current frame rate %4.1f fps.\n",
                        now - frame_start, server->max_framerate_);
          
          // Snap lastFrameTime to now to prevent infinite accumulated drift
          // resulting from hardware limits (e.g. camera is just too slow)
          lastFrameTime = now;
        }
      }
      ...
      if (server->server_socket_ >= 0) {
        struct sockaddr_in client_addr;
        socklen_t client_len = sizeof(client_addr);
        client = accept(server->server_socket_, (struct sockaddr *)&client_addr,
                        &client_len);
        if (client >= 0) {
          streamer = new OV2640Streamer(
              client, cam); // Our streamer for UDP/TCP based RTP transport
          session = new CRtspSession(
              client, streamer); // Our threads RTSP session and state

          lastFrameTime =
              esphome::millis(); // Initial time for image streamming
        }
      }
    }
...
void Esp32camRtsp::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  External Clock: Pin:%d Frequency:%u", config.pin_xclk,
                xclk_freq_hz_);
